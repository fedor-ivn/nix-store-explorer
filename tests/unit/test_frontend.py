...
from src.frontend import TIMEOUT, base_url, register_page, login_page, logout, main_page
...
def test_login_page_success():
    with patch("src.frontend.requests.post") as mock_post:
        with patch("src.frontend.st.button") as mock_button:
            with patch("src.frontend.st.text_input") as mock_text_input:
                with patch("src.frontend.st.success") as mock_success:
                    with patch("src.frontend.st.session_state") as mock_session_state:
                        ...
                        mock_post.assert_called_once_with(f"{base_url}/auth/jwt/login",
                        ...
                        mock_session_state.__setitem__.assert_called_with("Set_cookies", {"fastapiusersauth": "token"})
        ...
        with patch("src.frontend.st.button") as mock_button:
            with patch("src.frontend.st.text_input") as mock_text_input:
                with patch("src.frontend.st.error") as mock_error:
                    ...
                    mock_error.assert_called_once_with("Login failed. Please check your credentials.")
...
def test_logout_page():
    with patch("src.frontend.st.session_state") as mock_session_state:
        with patch("src.frontend.st.success") as mock_success:
            with patch("src.frontend.requests.post") as mock_post:
                with patch("src.frontend.st.button") as mock_button:
                    with patch("src.frontend.cookie_manager") as mock_cookie_manager:
                        mock_button.return_value = True
                        mock_post.return_value.status_code = 204
                        ...
                        mock_post.assert_called_once_with(f"{base_url}/auth/jwt/logout", cookies={"fastapiusersauth": "token"}, timeout=TIMEOUT)
            ...
            with patch("src.frontend.requests.post") as mock_post:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        ...
                        mock_post.assert_called_once_with(f"{base_url}/store/store_name", cookies=cookies, timeout=TIMEOUT)
...
def test_main_page_get_store():
    ...
    with patch("src.frontend.st.button") as mock_button:
        with patch("src.frontend.st.write") as mock_write:
            with patch("src.frontend.requests.get") as mock_get:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        mock_button.side_effect = button_side_effect
                        ...
                        mock_get.assert_called_once_with(f"{base_url}/store/store_name", cookies=cookies, timeout=TIMEOUT)            
                        calls = [call("Store ID: 1"), call("Store name: store_name"), call("Owner ID: 1")]
...
def test_main_page_delete_store():
    ...
    with patch("src.frontend.st.button") as mock_button:
        with patch("src.frontend.st.success") as mock_success:
            with patch("src.frontend.requests.delete") as mock_delete:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        mock_button.side_effect = button_side_effect
                        ...
                        mock_delete.assert_called_once_with(f"{base_url}/store/store_name", cookies=cookies, timeout=TIMEOUT)
            ...
            with patch("src.frontend.requests.post") as mock_post:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        ...
                        mock_post.assert_called_once_with(f"{base_url}/store/store_name/package/package_name",
                                            cookies=cookies,
                                            timeout=None,)
...
def test_main_page_add_package_warning():
    ...
    with patch("src.frontend.st.button") as mock_button:
        with patch("src.frontend.st.warning") as mock_warning:
            with patch("src.frontend.st.text_input") as mock_text_input:
                mock_button.side_effect = button_side_effect
                mock_text_input.side_effect = text_input_side_effect
                ...
                mock_warning.assert_called_once_with("Please enter both store name and package name")
            ...
            with patch("src.frontend.requests.delete") as mock_delete:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        ...
                        mock_delete.assert_called_once_with(f"{base_url}/store/store_name/package/package_name",
                                            cookies=cookies,
                                            timeout=TIMEOUT,)
            ...
            with patch("src.frontend.requests.get") as mock_get:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        ...
                        mock_get.assert_called_once_with(f"{base_url}/store/store_name/package/package_name",
                                            cookies=cookies,
                                            timeout=TIMEOUT,)
                        calls = [call("Package package_name in store store_name:"), 
                                 call("Present: True"), 
                                 call("Closure Size: 1")]
                        mock_write.assert_has_calls(calls)
            ...
            with patch("src.frontend.requests.get") as mock_get:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        mock_button.side_effect = button_side_effect
                        mock_get.return_value.status_code = 200
                        paths_difference = {"absent_in_store_1": ["path_1"], "absent_in_store_2": ["path_2", "path_3"]}
                        ...
                        mock_get.assert_called_once_with(f"{base_url}/store/store_name/difference/other_store_name",
                                            cookies=cookies,
                                            timeout=TIMEOUT,)
                        
                        calls = [call("Paths absent in store 1:"),
                                    call("path_1"),
            ...
            with patch("src.frontend.requests.get") as mock_get:
                with patch("src.frontend.st.session_state") as mock_session_state:
                    with patch("src.frontend.st.text_input") as mock_text_input:
                        mock_button.side_effect = button_side_effect
                        mock_get.return_value.status_code = 200
                        closures_difference = {"difference": [
                            {"package_name": "name_1", "version_update": {"old": "1.0.0", "new": "2.0.0"}, "size_update": 10},
                            {"package_name": "name_2", "version_update": {"old": "1.0.0", "new": "2.0.0"}, "size_update": 10},]}
                        ...
                        mock_get.assert_called_once_with(f"{base_url}/store/store_name/package/package_name/closure-difference/other_store_name/other_package_name",
                                            cookies=cookies,
                                            timeout=TIMEOUT,)
                        
                        calls = [call('Closure difference:'),
