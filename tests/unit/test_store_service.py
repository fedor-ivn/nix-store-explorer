...
async def test_get_paths_difference(store_service):
    ...
    with patch("src.services.stores.core_logic.get_paths") as mock_get_paths:
        ...
        paths_diff_1, paths_diff_2 = await service.get_paths_difference(store_1, store_2, user)
...
async def test_closures_difference(store_service):
    ...
    with patch("src.services.stores.core_logic.get_closure") as mock_get_closure:
        mock_get_closure.side_effect = [["package1", "package2"], ["package2", "package3"]]
...
async def test_get_packages_meta(store_service):
    ...
    with patch("src.services.stores.core_logic.get_closure_size") as mock_get_closure_size:
        ...
        mock_get_closure_size.assert_called_once_with(service.stores_path / "1" / store_name, package_name)
